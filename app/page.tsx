...
const LEADS_WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbzZDSkpP_QI-vZPLm66ahz9KPY33JODfhLcKEmylGL-SzU_P7NmcsP-iD6vuj8DWvmtCw/exec";
...
export default function Home(){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      await fetch(LEADS_WEBHOOK_URL,{method:"POST",mode:"no-cors",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({...lead,source:"Сайт «Автоматизация бизнес-процессов»",status:"Новая",submittedAt:new Date().toISOString()})});
      form.reset(); setState("sent");
    }catch{setState("error")}
  }
  return <main>
    <header><div className="container header">
      <a className="brand" href="#top"><Image src="/automation-cube.png" width={42} height={42} alt="" unoptimized/><span><b>Автоматизация</b><small>бизнес-процессов</small></span></a>
    ...
    <section className="hero" id="top"><div className="container heroGrid">
      ...
      <div className="visual"><Image src="/automation-cube.png" width={720} height={720} alt="Google Таблицы, n8n и бот в единой системе" priority unoptimized/><div className="orbit"><i className="chip c1">● Google Таблицы</i><i className="chip c2">● n8n</i><i className="chip c3">● Боты</i><i className="chip c4">● ИИ-агенты</i></div></div>
    ...
    <section className="section contact" id="contact"><div className="container contactGrid"><div><p className="kicker">Обсудим задачу</p><h2>Какой процесс отнимает больше всего времени?</h2><p>Опишите ситуацию — я предложу подходящие варианты автоматизации.</p><div className="social"><a href="https://t.me/VasiliyPF" target="_blank" rel="noreferrer"><b>➤</b><span><small>Написать в</small>Telegram</span>↗</a><a href="https://vk.com/autobizvb" target="_blank" rel="noreferrer"><b>VK</b><span><small>Написать во</small>ВКонтакте</span>↗</a></div></div>
      <form onSubmit={submit}><div className="formRow"><label>Ваше имя<input name="name" placeholder="Как к вам обращаться" required/></label><label>Компания или сфера<input name="company" placeholder="Например, страхование"/></label></div><div className="formRow"><label>Способ связи<select name="contactMethod"><option>Telegram</option><option>ВКонтакте</option><option>Телефон</option><option>Email</option></select></label><label>Контакт<input name="contact" placeholder="@username или телефон" required/></label></div><label>Услуга<select name="service"><option>Бесплатный аудит</option><option>Google Таблицы и Apps Script</option><option>Автоматизация в n8n</option><option>Telegram- или VK-бот</option><option>ИИ-бот или ИИ-агент</option><option>Сопровождение — от 3 000 ₽/месяц</option></select></label><label>Кратко опишите задачу<textarea name="message" rows={5} placeholder="Что делается вручную и какой результат нужен?" required/></label><label className="consent"><input type="checkbox" name="consent" required/>Согласен на обработку данных для ответа на заявку</label><button className="btn primary" disabled={state==="sending"}>{state==="sending"?"Отправляем…":"Отправить заявку ↗"}</button>{state==="sent"&&<p className="success">Заявка отправлена. Я свяжусь с вами.</p>}{state==="error"&&<p className="error">Не удалось отправить. Напишите в <a href="https://t.me/VasiliyPF">Telegram</a>.</p>}</form>
    </div></section>
    <footer><div className="container"><div className="brand"><Image src="/automation-cube.png" width={40} height={40} alt="" unoptimized/><span><b>Автоматизация</b><small>бизнес-процессов</small></span></div><p>Google Таблицы · Apps Script · n8n · Боты · ИИ</p><div><a href="https://t.me/VasiliyPF">Telegram</a><a href="https://vk.com/autobizvb">ВКонтакте</a></div></div></footer>
  </main>
}
